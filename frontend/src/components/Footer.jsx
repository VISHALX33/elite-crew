        ...
        <div className="max-w-7xl mx-auto px-6 py-8">
          <div className="grid grid-cols-3 md:grid-cols-4 gap-8">
            ...
            <div className="space-y-4">
              <Link to="/" className="flex items-center">
                <span className="text-2xl font-bold bg-gradient-to-r from-orange-600 to-green-600 bg-clip-text text-transparent">QuickHaat</span>
              </Link>
              ...
              <div className="space-y-2">
                {navLinks.map((link) => (
                  <Link
                    key={link.to}
                    to={link.to}
                    className={`block text-sm hover:text-orange-600 transition-colors ${location.pathname === link.to ? 'text-orange-600 font-medium' : 'text-gray-600'
                      }`}
                  >
                    {link.label}
                  </Link>
          ...
          <div className="mt-8 pt-8 border-t border-gray-200 flex flex-col md:flex-row justify-between items-center">
            <p className="text-sm text-gray-500">
              &copy; {new Date().getFullYear()} QuickHaat. All rights reserved.
            </p>
        ...
        <div className="flex justify-around items-center py-2">
          {mobileLinks.map((link) => (
            <Link
              key={link.to}
              to={link.to}
              className={`flex flex-col items-center p-2 rounded-lg transition-colors ${location.pathname === link.to
                ? 'text-orange-600 bg-orange-50'
                : 'text-gray-600 hover:text-orange-600'
                }`}
            >
              <div className={`p-1 rounded-full ${location.pathname === link.to ? 'bg-orange-100' : ''
                }`}>
                {link.icon}
              </div>
